...
def seperate_nodule(origin, filt, unet_mask ,nodules):
    selem = ball(2)
    segmented_ct_scan = unet_mask.copy()
    #segmented_ct_scan = segmented_ct_scan0.copy()
    segmented_ct_scan[segmented_ct_scan >= 0.999999999] = 1
    segmented_ct_scan[segmented_ct_scan < 0.999999999] = 0
    ...
    for r in rr:
        if r.area < 22 or r.area > 36000:    #the condinate nodule is too small or too big and skip it
            continue
...
train_path = '/media/izm/Entertain/fjj/train_subset05-08/'
...
for fcount, img_file in enumerate(tqdm(file_list)):
    ...
    if mini_df.shape[0]>0:                      # some files may not have a nodule--skipping those 
        ...
        org_cubes, segment_cubes, labels, centers = seperate_nodule(img_array, delt_scan, unet_sgm, nodules)
