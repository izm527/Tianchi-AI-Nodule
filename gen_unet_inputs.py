...
def extract_nodule(ct_scan, masks, ll, hh):
	...
	#mask_scan[mask_scan < threshold] = -1000
        ...
        unet_mask[unet_mask > -440] = 1
        unet_mask[unet_mask < 0 ] = 0

        return mask_scan, mask_scan[ll:hh], np.max(unet_mask)
...
train_path = '/media/izm/Entertain/fjj/why/'
...
out_path = '/media/izm/Entertain/fjj/output/'
...
for fcount, img_file in enumerate(tqdm(file_list)):
	...
	#np.save('delt_scan', delt_scan)
        ...
        for node_idx, cur_row in mini_df.iterrows():			
            ...
            count += 1
            idx_str = '-'+str(count)
	    res, unet_mask, chk = extract_nodule(segmented, masks, ll, hh)
            if chk < 1:
                print 'nodule null skip!'
                continue
            print "ll---->hh", ll, hh
            plot_3d(res, name + idx_str, -400 )
