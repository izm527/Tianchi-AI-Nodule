...
train_path = '/media/izm/Entertain/fjj/train_subset12-14/'
file_list = glob(train_path + "*.mhd")
df_node = pd.read_csv('annotations.csv')
out_path = '/media/izm/Docs/FJJ-vessels/vessel-for-unet/'
...
#-----------------some parameters------------#
thick = 32
zfile = open("output/vessel/z-area-12-14.txt", 'w')
cnt = 0
for fcount, img_file in enumerate(tqdm(file_list)):
    ...
    if mini_df.shape[0]>0:                      # some files may not have a nodule--skipping those 
        ...
        delt_scan, vessel = filt_vessels(segmented)
