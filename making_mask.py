...
def filt_vessels(segmented_ct_scan0):
    print "filting vessels!"
    selem = ball(2)

    #segmented_ct_scan0 = ct_scan.copy()
    #segmented_ct_scan0 = segmented_ct_scan0.transpose(2,1,0)

    linking_scan = segmented_ct_scan0.copy()
    linking_scan[linking_scan >= -313] = 1
    linking_scan[linking_scan < -313] = 0
    ...
    return binary_closing(segmented_ct_scan0, selem)
...
file_list = glob(train_path + "*168.mhd")
#file_list = ['/home/izm/work/fjj/data/train_subset00/LKDS-00066.mhd']
df_node = pd.read_csv('data/csv/train/' + 'annotations.csv')
out_path = 'output/'
...
for fcount, img_file in enumerate(tqdm(file_list)):
    ...
    if mini_df.shape[0]>0:                      # some files may not have a nodule--skipping those 
	    ...
	    res, unet_mask = extract_nodule(img_array, masks, ll, hh)
            print "ll---->hh", ll, hh
            #plot_3d(res, 0)
            #plot_3d(delt_scan, -200)
            #plot_3d(unet_mask, 0)
	    #plot_3d(delt_scan[ll:hh], -200)
	    #plot_3d(delt_scan, -320)
            #plot_3d(res, -330)
            #raw_input("finish")
            name = img_file.split('/')[-1] +str(count)
            #plot_3d(unet_mask, 0)
            #plot_3d(res, 0)
            plot_3d(delt_scan, -280)
	    #np.save(out_path+'unet-lb-'+name, unet_mask)
	    #np.save(out_path+'unet-in-'+name, delt_scan[ll:hh])
