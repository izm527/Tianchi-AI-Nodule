...
def load_data(p, q):
    lb_prefix = "lb-1-1000/unet-lb-"
    input_prefix = "in-1-1000/unet-in-"
    file_name = "label.txt"
    fp = open(file_name)
    lines = fp.readlines()
    valid_scan_name = []
...
data, lb = load_data(0,380)
shuffle_idx = range(data.shape[0])
data = data[shuffle_idx]
lb = lb[shuffle_idx]

test_data, test_lb = load_data(380,450)
...
def main():
    ...
    model.load_weights('model/'+'0-370_batch-13_eph-5')
