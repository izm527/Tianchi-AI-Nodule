...
def seperate_nodule(origin, filt, unet_mask):
    ...
    print 'condinate nodule:', len(cube)
...
lb = np.load('lb-LKDS-00020.mhd.npy')
res = np.load('LKDS-00020.mhd.npy')
...
#train_path = 'dir05-08/'
train_path = 'center/'
cc_path = '/media/izm/Mix/fjj-test/all/'
#train_path = '/media/izm/Entertain/fjj/train_subset00-04/'
file_list = glob(train_path + "*.npy")
file_list.sort()
#file_list = ['/home/izm/work/fjj/data/train_subset00/LKDS-00066.mhd']
print 'start!'
res_file = open('confirm', 'w')
for fcount, img_file in enumerate(tqdm(file_list)):
	ctr = np.load(img_file)
	print img_file
    	name = img_file.split('/')[-1]
	prob = np.load('prob/'+name)
	name = name.split('.')[0]

	img_file = cc_path+name+'.mhd'
	...
	for i in range(ctr.shape[0]):
		if prob[i] <= 0.0:
			continue
		point = ctr[i]*spacing+origin
		line = (name + '\t' + str(point[0]+0.0001) + '\t'+ str(point[1]+0.0001) + '\t'+str(point[2]+0.0001) + '\t'+ str(prob[i][0]) + '\n')
		print line
		res_file.write(line)
